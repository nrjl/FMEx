...
mean_value = 6.0           # Mean value of the field for GP
num_blobs = 28          # Number of blobs in each field
peak_range = [5.0,8.0]      # Low and high peak values for map blobs
spread_range = [6,10]    # Low and high spread distances for map blobs
poly_cost_radius = 15
...
def sample_value(fm, pc, x, y):
    ...
    ccu = fm.cost_update_new(pc.set_update(x, y, 0.1*std))
